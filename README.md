...
* cfy 始终是独立模块，保留独立仓库、`cfy` 命令和更新周期；Sing-box 菜单只负责安装和调用，不会把 cfy 的节点生成逻辑合并进 Sing-box 脚本。
* 在支持该联动的最终版 Sing-box 中，可从主菜单进入“Cloudflare 节点优选（cfy）”。首次使用时会安全安装 cfy，随后以前台交互方式运行；退出 cfy 后会返回 Sing-box 菜单。
* 不通过 Sing-box 菜单时，仍可直接运行 `cfy`、`cfy -c` 和 `cfy --update`，其含义和独立更新方式保持不变。
* cfy 安装或运行失败不会重启或改动 sing-box、Argo、Nginx，不会修改端口或已有基础节点；最近一次成功的优选结果会按 cfy 现有事务语义保留。
* cfy 成功运行后，仍按现有文件契约更新 cfy 优选结果，并重新生成包含基础节点和有效优选节点的综合订阅。
